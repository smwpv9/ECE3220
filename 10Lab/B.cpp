...
public:
	//Constructors
	Signal( );
	Signal( int fileNum ) throw( int, string );
	Signal( string fileName ) throw(string);
	//Destructor
	~Signal( );
	//Other
	void operator+( double off );
	void operator*( double mult ) throw(wrong);
...
void manual( );
Signal operator+( Signal &left, Signal &right ) throw(string);
		...
		if( argv[ 1 ][ 0 ] == '-' )//is the beginning a dash
		{
			if( argv[ 1 ][ 2 ] == '\0' )// is the arg only 2 characters
			{
				...
				case 'n': // Is a file number
					...
					{
						cerr << "File number: " << n << " is not within [1,99]." << endl;
						cerr << "Exiting!\n" << endl;
						return 2;
					}
					catch( string s )
	...
	do {
		...
		case 'n': //load new file
			...
			else //Then choice has to equal '2'
			{
				...
				{
					cerr << "Something just went completely wrong.\n" << endl;
				}
			}
			break;
...
Signal::Signal( int fileNum ) throw( int, string )
{
	...
	if( !in.is_open( ) ) // see if the file is open
		throw "File: " + fileName + " is unavaible currently.";
...
void Signal::operator*( double scale ) throw( wrong)
{
	...
	{
		cerr << "Will not scale because" << endl;
		throw;
	}
...
Signal operator+( Signal &left, Signal &right ) throw( string )
{
	Signal one;
	if(! (left == right) )
		throw "Signals of different lengths cannot be added";
